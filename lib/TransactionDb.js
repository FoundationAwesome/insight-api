...
TransactionDb.prototype.fillScriptPubKey = function(txouts, cb) {
  ...
  async.eachLimit(txouts, CONCURRENCY, function(txout, a_c) {
    self.fromIdInfoSimple(txout.txid, function(err, info) {
      ...
      //console.log('info in fillScriptPubKey is ',info )
      //console.log(info.vout[txout.index].scriptPubKey)
      txout.scriptPubKey = info.vout[txout.index].scriptPubKey.asm;
      return a_c();
    });
  }, function() {
